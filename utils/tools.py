...
def to_device(data, device, diffusion_hyperparams=None):
    ...
    if len(data) == 6:
        ...
        return (ids, raw_texts, speakers, texts, src_lens, max_src_len), diffusion_hyperparams
...
def transforme_mel(diffusion_steps, diffusion_hyperparams, mel_0, z=None):
    steps = 1 + diffusion_steps
    if z == None:
        z = std_normal(mel_0.shape)
    transformed_X = torch.sqrt(diffusion_hyperparams["Alpha_bar"][steps]) * mel_0 + torch.sqrt(1 - diffusion_hyperparams["Alpha_bar"][steps]) * z
