...
from utils.model import get_model, get_vocoder, get_param_num, train_model, test_model
from utils.tools import log, synth_one_sample, to_device, calc_diffusion_hyperparams
    ...
    while True:
        ...
        for batchs in loader:
            for batch in batchs:
                ...
                if step % val_step == 0:
                    model.eval()
                    message = evaluate(model, step, configs, diffusion_hyperparams, val_logger, vocoder)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-m", "--model_config", type=str, default='config/LJSpeech/model.yaml', required=False, help="path to model.yaml"
    )
    parser.add_argument(
        "-t", "--train_config", type=str, default='config/LJSpeech/train.yaml', required=False, help="path to train.yaml"
    )
    parser.add_argument(
